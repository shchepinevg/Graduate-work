...
class ToR:
    ...
    def get_optimMeth(self):
        if self.info["is_function"] == 1:
            str_optim = "{} {} {} {}".format(str(self.info["is_function"]), self.info["optimization_meth"],
                                       str(self.info["N"]), str(self.info["min_or_max"]))
            return str_optim
        elif self.info["is_function"] == 2:
            str_optim = "{} {} {} {} {} {} {}".format(str(self.info["is_function"]), self.info["optimization_meth"],
                                                self.info["meta_optim_meth"], str(self.info["N"]),
                                                str(self.info["meta_N"]), str(self.info["k"]), str(self.info["min_or_max"]))
            return str_optim

    def get_paramMeth(self):
        if self.info["isRecommend"] == 1:
            return "1"
        elif self.info["isRecommend"] == 2:
            str_param = "2 "
            params = []
            if self.info["is_function"] == 1:
    ...
    def get_path(self):
        ...
        path = os.path.join(path_to_dir, relative_path)
